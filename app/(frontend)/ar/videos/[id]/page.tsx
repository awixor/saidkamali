...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const video = await payload.findByID({ collection: "videos", id, depth: 1 });
    return {
      title: `${video.title} - الشيخ سعيد الكملي`,
      description: video.description || `${video.title} من شرح موطأ الإمام مالك`,
      openGraph: {
        title: video.title,
        description: video.description || `${video.title} من شرح موطأ الإمام مالك`,
...
export default async function VideoPage({ params }: Props) {
  ...
  const book =
    typeof video.book === "object" ? video.book : null;
